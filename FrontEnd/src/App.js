...
function App() {
  ...
  const peticionMigrate = async () => {
    var f = new FormData();
    f.append("METHOD", "MIGRATE");


    await axios.post(baseUrl + '/migracion', f)
      .then(response => {
        console.log("Migracion Creada")
        // console.log(response.data.message);
      })
      .catch(error => {
        console.log(`Error al ejecutar la migración: ${error.message}`);
      })
      .finally(() => {
      });
  };
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <button className="btn btn-success" onClick={() => peticionMigrate()}>Ejecutar Migración</button>
      <br /><br />
      <table className="table table-striped">
        ...
        <tbody>
          {Array.isArray(data) && data.map(framework => (
            <tr key={framework.id}>
              <td>{framework.id}</td>
